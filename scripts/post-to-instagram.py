...
Usage:
  python3 post-to-instagram.py --images "url1,url2,url3" "Caption text"
  python3 post-to-instagram.py --dry-run --images "url1,url2" "Caption text"
...
def _handle_error(resp, step_name):
    ...
    if resp.status_code == 401:
        ...
        print("Regenerate token at Meta Developer Portal and update INSTAGRAM_ACCESS_TOKEN.", file=sys.stderr)
        sys.exit(1)
...
def post_carousel_to_instagram(image_urls: list[str], caption: str, dry_run: bool = False) -> str:
    ...
    if not account_id:
        ...
        sys.exit(1)
    if not token:
        print("Error: INSTAGRAM_ACCESS_TOKEN env var not set", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    if not images_arg:
        print("Usage: python3 post-to-instagram.py [--dry-run] --images \"url1,url2,...\" \"Caption text\"", file=sys.stderr)
        sys.exit(1)
